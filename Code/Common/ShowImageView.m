...
- (void)configScrollViewWith:(NSInteger)clickTag andAppendArray:(NSArray *)appendArray{
    ...
    for (int i = 0; i < appendArray.count; i ++) {
        ...
        if ([[appendArray objectAtIndex:i] isKindOfClass:[UIImage class]])
        {
            imageView.image = [appendArray objectAtIndex:i];
        }
        else if ([[appendArray objectAtIndex:i] containsString:@"http"])
        {
            NSURL *imageURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@",[appendArray objectAtIndex:i]]];
            NSString *placeHolder = [NSString stringWithFormat:@"%@placeholder_1b1", PREFIX_NAME];
            [imageView sd_setImageWithURL:imageURL placeholderImage:[UIImage imageNamed:placeHolder]];
        }
        else
        {
            imageView.image = [UIImage imageNamed:[appendArray objectAtIndex:i]];
        }
        
        
        //        imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",[appendArray objectAtIndex:i]]];
        //        NSURL *imageURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@",[appendArray objectAtIndex:i]]];
        //        [imageView sd_setImageWithURL:imageURL placeholderImage:[UIImage imageNamed:@"CircleDownImgD"]];
